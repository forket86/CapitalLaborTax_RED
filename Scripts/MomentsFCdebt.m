...
UHH=u(c)-v(labor);
V_FCdebt=zeros(T-CUTEND_WELFARE,1);
VtauL_FCdebt=zeros(T-CUTEND_WELFARE,1);
VtauK_FCdebt=zeros(T-CUTEND_WELFARE,1);
VHH_FCdebt=zeros(T-CUTEND_WELFARE,1);
for t=1:T-CUTEND_WELFARE
    gridC=0:length(U(t:t+CUTEND_WELFARE))-1;
    V_FCdebt(t)=beta.^gridC*U(t:t+CUTEND_WELFARE)';
    VtauL_FCdebt(t)=beta.^gridC*UtauL(t:t+CUTEND_WELFARE)';
    VtauK_FCdebt(t)=beta.^gridC*UtauK(t:t+CUTEND_WELFARE)';
    VHH_FCdebt(t)=beta.^gridC*UHH(t:t+CUTEND_WELFARE)';
end
T0=50;Tend=T;
